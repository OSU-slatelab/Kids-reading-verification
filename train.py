...
class Trainer(object):
    def __init__(self, args, data, device, optimizer, normalizer=None, data_valid=None, data_test=None):
        ...
        if self.args.pretrain:
            self.scheduler = torch.optim.lr_scheduler.OneCycleLR(self.optimizer, max_lr=0.0003, total_steps=args.nsteps, anneal_strategy='linear', pct_start=0.1)
    ...
    def evaluate(self, model, loader):
        ...
        y_pred = []
        y_true = []
        for X, lens, lmax, tbatch, text_tgt, merge_idx, cls_tgt in tqdm(loader):
            lens_norm = [1.*(x/lmax) for x in lens]
            sbatch = self.norm(X, torch.tensor(lens_norm).float(), epoch=1000)
            sbatch = list_batch(sbatch, lens, lmax)
            sbatch, tbatch = load2gpu(sbatch, self.device), load2gpu(tbatch, self.device)
            if self.args.pretrain:
                ...
                label = load2gpu(cls_tgt, self.device)
            with torch.no_grad():
                pred, _, _ = model(sbatch, tbatch, merge_idx)
            y_pred.extend(torch.max(pred, dim=1)[1].cpu().tolist())
            y_true.extend(label.cpu().tolist())
        if self.args.pretrain:
    ...
    def pretrain(self, model, logger, nsteps, log_after, val_after):
        ...
        while True:
            ...
            for X, lens, lmax, tbatch, text_tgt, merge_idx, cls_tgt in tqdm(self.loader):
                model.train()
                steps += 1
                lens_norm = [1.*(x/lmax) for x in lens]
                sbatch = self.norm(X, torch.tensor(lens_norm).float(), epoch=epoch-1)
                sbatch = list_batch(sbatch, lens, lmax)
                #sbatch = [self.aug(x) for x in sbatch]  TODO adding specaugment
                sbatch, tbatch = load2gpu(sbatch, self.device), load2gpu(tbatch, self.device)
                label = load2gpu(text_tgt, self.device)
                pred, _, _ = model(sbatch, tbatch, merge_idx)
    ...
    def detect(self, model, logger):
        ...
        while rpat > 0:
            ...
            for X, lens, lmax, tbatch, text_tgt, merge_idx, cls_tgt in tqdm(self.loader):
                ...
                pred_cls, pred_tok, _ = model(sbatch, tbatch, merge_idx)
