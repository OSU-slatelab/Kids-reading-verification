...
class ReaderSB(nn.Module):
    def __init__(self, embed_dim, vocab_size, listener_dim, dropout=0.1):
        super(ReaderSB, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim)
        self.decoder = AttentionalRNNDecoder('lstm', 'keyvalue', listener_dim, listener_dim, 1, listener_dim, embed_dim, normalization='layernorm')

    def forward(self, input_x, input_s, lens): # bsz, seq_len
        lens_org = (copy.deepcopy(lens)).long()
        lens = lens.cpu()
        lens = lens + input_x.size(1) - lens.max()
        embed_in = self.embedding(input_x)
        out_speller, _ = self.decoder(embed_in, input_s.permute(1,0,2), wav_len)
        # pack sequence
        pack = pack_padded_sequence(embed_in, lens, batch_first=True, enforce_sorted=False)
        # forward pass - LSTM
        self.encoder.flatten_parameters()
